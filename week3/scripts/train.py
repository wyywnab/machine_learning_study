...
from models.cnn_small import SmallCNN, SmallCNNAttention
...
def get_data_loaders(batch_size, seed, num_workers):
    ...
    train_set = EMNIST("..\..\emnist_data", split="letters", transform=train_set_transform, download=True, train=True)
    ...
    test_set = EMNIST("..\..\emnist_data", split="letters", transform=test_set_transform, download=True, train=False)
...
def main():
    ...
    if config["model"] == "SmallCNN":
        net = SmallCNN()
    elif config["model"] == "SmallCNNAttention":
        net = SmallCNNAttention()
